...
Работает с любым AI-агентом, поддерживающим формат SKILL.md:

| Агент | Установка |
|-------|-----------|
| **[Claude Code](https://claude.com/product/claude-code)** | `cp -r osint/ ~/.claude/skills/osint/` |
| **[OpenClaw](https://github.com/openclaw/openclaw)** | Скопировать в `skills/` рабочей директории |
| **[Codex](https://developers.openai.com/codex/app)** | Указать путь к `osint/SKILL.md` в конфиге агента |
| **[OpenCode](https://github.com/anomalyco/opencode)** | Скопировать в `skills/` проекта |
| **Любой SKILL.md агент** | Положить `osint/` туда, где агент читает skills |

Skill использует стандартные инструменты (`bash`, `curl`, `node`, `python3`) и инструкции в Markdown — никакого vendor lock-in.
